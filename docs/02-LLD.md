...
| E65 | (S8, client UI) The ⌘K command palette (03-UI §4.8). `ui/commands.ts` is the palette's contents as PURE data: `paletteCommands(actions)` returns sixteen rows in five groups (Navigate · Document · Collaboration · History · Debug), each `{ group, title, icon, value?, run }`, and `filterCommands(commands, query)` is the case-insensitive title-or-group substring filter. Navigate exposes Documents, Editor, History screen, and Settings through the same actions as the rail. Every `run` is a behaviour the shell supplies through `PaletteActions`; the file reaches for no DOM, runner or router, so `commands.test.ts` (coverage-gated, `src/ui/*.ts` is held to the client gate) proves each row runs exactly its wired action. `ui/CommandPalette.tsx` renders its own top-bar trigger button plus a REAL `<dialog>`: Ctrl/Cmd+K toggles it, typing filters, ArrowUp/Down move the selection, Enter or a click runs, Esc and a click outside dismiss it. | 03-UI §4.8 and the "port the prototype" rule (the `.cmdk` markup and section-12 JS are ported). Splitting contents (pure, tested) from the dialog (`.tsx`, coverage-excluded, proven behaviourally) keeps the wiring provable without rendering the shell. |
